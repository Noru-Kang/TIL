...
- 6강: [데이터결합 혼합](#6-데이터결합-혼합)

- 7강: [데이터결합 및 유니온](#7-데이터결합-및-유니온)

- 8강: [라이브 및 추출](#8-라이브-및-추출)
...
```
라이브 : 실시간 처리   
추출 : 일정시간마다 데이터 업데이트, 데이터가 테블로 서버로 복사가됨   -> 대규모 데이터셋에 적절
```
...
> **🧞‍♀️ 관계를 사용하는 경우와 조인을 사용하는 경우의 차이가 무엇인가요?**
...
> **🧞‍♀️ 주문 중 '반품된' 데이터만을 가지고 분석을 진행하려고 합니다. 이때 해당 Sample store 데이터셋에서 어떤 시트 둘을 조인(혹은 릴레이션)해야 할까요? 또, 조인 시 키는 무엇일까요?**

```
"주문 Id"를 키로 inner join
```

데이터 원본 창의 캡쳐를 첨부해주세요.
몇 개의 주문이 반품되었다고 표시되나요?

![주문시트](./img/inner_join.png)   
22필드 3027개행 -> 3027개
...
데이터의 테이블 구조가 동일해야 함.
즉, 각 테이블 필드 수가 같고 관련 필드의 필드 이름과 유형이 일치해야 함.
...
라이브는 데이터가 실시간 업데이트 반영된다. 
추출은 tableau 서버에 데이터가 복제된다.


> 참고

온프레미스 데이터: 기업이나 조직이 자체적으로 보유한 물리적 서버나 데이터 센터 내에서 직접 관리하는 데이터로, 조직 내의 하나 이상 서버에서 통제됩니다. 본 서버들은 기업의 데이터센터나 물리적 위치에 설치되어 있으며, 네트워크를 통해 해당 데이터에 접근하고 관리할 수 있습니다.
